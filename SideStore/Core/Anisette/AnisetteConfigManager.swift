...
public actor AnisetteConfigManager {
    ...
    public func makeRequestHeaders() -> AnisetteRequestHeaders {
        ...
        // Apple rejects the legacy Xcode application identity in GSA client info.
        // Preserve the device/OS description and normalize saved or imported overrides.
        let clientInfo = (config.clientInfo.isEmpty ? AppConstants.Anisette.defaultClientInfo : config.clientInfo)
            .replacingOccurrences(of: #"(?<=\()com\.apple\.dt\.Xcode/[^\s()<>]+(?=\))"#,
                                  with: "com.apple.akd/1.0", options: .regularExpression)
